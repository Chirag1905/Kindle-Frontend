...
const ExamModal: React.FC<ExamModalProps> = ({
    ...
    const handleBatchesChange = (selectedBatches: number[]) => {
        setFormData((prev) => ({ 
            ...prev, 
                ...
                <div className="flex items-center justify-between mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
                    <div className="text-sm text-gray-500 dark:text-gray-400">
                        {(formData.selectedBatches || []).length > 0 ? (
                            <>
                                Exam will be conducted for {(formData.selectedBatches || []).length} batch(es)
                            </>
                        ) : (
                            'Select batches to continue'
                        )}
                    </div>
